(function () {
  ...
  const MARGIN_X = 34, MARGIN_TOP = 4;
  const HOUSES_H = 3.2 * M, GAP_HOUSES_SEATS = 6;
  const ROAD_PAD = 10, GAP_SEATS_FIELD = 6;
  ...
  function render() {
    ...
    // Drawn top (nearest the houses) to bottom (nearest the field), so the
    // row closest to the fireworks field ends up lettered "A".
    const rows = [...new Set(seats.map((s) => s.row))].sort().reverse();
    const seatsPerRow = Math.max(...seats.map((s) => s.number));

    const seatsWidth = seatsPerRow * SEAT_W + (seatsPerRow - 1) * SEAT_GAP;
    ...
    mapCaptionEl.textContent = `Approx. scale: seating spans ~${streetLengthM}m along the closed-off street, ${rows.length} rows deep (~${rowsDepthM}m).`;
    ...
    // Seat number ticks every 10 seats, above row A
    for (let n = 10; n <= seatsPerRow; n += 10) {
      const x = MARGIN_X + (n - 1) * (SEAT_W + SEAT_GAP) + SEAT_W / 2;
      const tick = el("text", { x, y: seatingY - 4, class: "map-tick" });
      tick.textContent = n;
    ...
    rows.forEach((row, rowIndex) => {
      ...
      const rowLabel = el("text", { x: MARGIN_X - 10, y: y + ROW_H - 3, class: "map-row-label", "text-anchor": "end" });
      ...
      seats
        ...
        .forEach((seat) => {
          const x = MARGIN_X + (seat.number - 1) * (SEAT_W + SEAT_GAP);
          const rect = el("rect", {
